...
/** @addtogroup c_inidb                Conf/ini Dateiverarbeitung*//** @{ @} */
...
#if (!defined IS64BIT) && (defined __linux) && (defined __x86_64__)
#define IS64BIT
#endif
...
tools_xtrn unsigned int m_BUILDCOUNT;           ///< BuildCount (wird nur hochgezählt)
tools_xtrn char*   m_BUILDDATE;                 ///< BuildDate (selbstredent)
...
tools_xtrn int lmsg_setup(int do_what, int parm, char *name); ///< Einen MsgLevel initialisieren
...
void strstrip(char *SRC, char rm);                                              // Zeichen wenn es am Anfag UND Ende ist entfernen
...
/**
 ...
 gibt die Anzahl den zuletzt angelegten Strings an.
 Wurde @ref strlstadd nicht aufgerufen, ist der Inhalt undefiniert
*/
tools_xtrn int strlstadd_num;
///@}
...
int strlstcount(char **listin);                                                 // Anzahl Items in einer strlst-Liste
char **strlstdelete(char **listin, int num);                                    // Löscht einzelnen Node in der Stringliste
int strlstcompare(const void *a, const void *b);                                // für qsort
int strlstsort(char **listin);                                                  // sortieren der strlst mit qsort
...
char *ctimeinfostr(time_t TIME, int locgmt);                                    // Worker für timeinfostr und gmtinfostr
char *gmtinfostr(time_t TIME);                                                  // das gleiche wie timeinfostr nur TIME ist gmt
...
char *CresolvPath(char *Kette);                         // Kette als fqn auflösen dh. alle . / .. auflösen in kompletten Pfad auflösen
...
char *RCfind_in_Section(void *RCNodes, char *var, char *sec, int num); // same, aber mit section
