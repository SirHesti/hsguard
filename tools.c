 ...
 ** 31.03.25 HS strlstadd_num neue globaler INT
 ** 29.04.25 HS atoul wieder reingenommen (Linux, kann Windows evtl. nicht)
 ** 28.05.25 HS timeinfostr überarbeitet (gmtinfostr,ctimeinfostr)
...
int InitTools ( int arguc, char *arguv[] , char* opts, ... )
{
    ...
    for (opt_pos=opts;;opt_pos++)                               // Zähler ist das in opt vermerkte
    {
        ...
        {                                                       // Nachrichten
          case 'b':
              m_BUILDCOUNT = va_arg(argpnt, int);
              m_BUILDDATE  = va_arg(argpnt, char *);
              break;
          case 'c':
...
#ifdef OS_LINUX
    // if nicht root dann HOME
    if (getuid()!=0) if (!tmp) tmp = getenv("HOME");		    // Wenn nicht dann wird "HOME=/home/pi" angenommen
...
/**
 @brief  einzelne definition ändern
 @return EXIT_FAILURE / EXIT_SUCCESS
 @param do_what     msg-level
 @param parm        parameter z.B. LOG_STDERR
 @param name        name des Levels für "[name] falsch"
...
int lmsg_setup( int do_what, int parm, char *name )
{
    lmsg_def[do_what].out=parm;
    lmsg_def[do_what].str=name;
    return EXIT_SUCCESS;
}
...
int lmsg_init(void)
{
    ...
    lmsg_format = 0;
    lmsg_setup(LMSG_SILENT    ,LOG_STDERR | LOG_LOGFILE | LOG_SYSLLOG | LOG_WINMSG,"SILENT");
    lmsg_setup(LMSG_CRITICAL , LOG_STDERR | LOG_LOGFILE | LOG_SYSLLOG,             "CRITICAL");
    lmsg_setup(LMSG_ERROR    , LOG_STDERR | LOG_LOGFILE,                           "ERROR");
    lmsg_setup(LMSG_WARN     , LOG_STDERR | LOG_LOGFILE,                           "WARN");
    lmsg_setup(LMSG_EVERY    , LOG_STDOUT | LOG_LOGFILE,                           "EVERY");
    lmsg_setup(LMSG_MOST     , LOG_STDOUT | LOG_LOGFILE,                           "MOST");
    lmsg_setup(LMSG_MAX      , LOG_STDOUT | LOG_LOGFILE,                           "MAX");

// ** statt ehemals (01.05.25) ....
...
/**
 @brief Ähnlich @ref strdelchar, aber nur entfernen wenn Anfang und Ende
 @param SRC die zu bearbeitende Zeichenkette
 @param rm char das entfernt werden soll

 Es wird ein einzelnes Zeichen am Anfagng UND Ende entfernt und zwar nur
 wenn es am Anfang UND Ende vorhanden ist. Für rm='"' oder "'" Fälle
...
void strstrip(char *SRC, char rm)
{
    int l;
//    int z;
    l=strlen(SRC);
...
char **strlst(char *strin)
{
	...
	for (strnum=0;;strnum++)
    {
        ...
        lst[strnum+1]=NULL;                                                     // Abschluss mit NULL
        if (!piece) break;
	}
...
/**
 ...
 @return Pointer auf die geänderte Stringliste
         geändert wird außerdem strlstadd_num (globale variable), die
         die die Anzahl der Elemente in der Liste zeigt.
 @param listin Pointer auf die "Stringliste"
 @param newitem anzufügendes pointer hinzufügen

 Sowohl die Liste als auch die Argumente müssen mit free freigegeben werden.
 Da der Rückgabewert die Liste ist kann die letzte Addition in der
 aktuellen Liste inf @ref strlstadd_num abgelesen werden.
...
char **strlstadd(char** listin, char *newitem)
{
    if (listin)
    {
        for (strlstadd_num=0;listin[strlstadd_num];strlstadd_num++);
        listin = realloc(listin,sizeof(char*) * (strlstadd_num+2));
    }else{
        strlstadd_num=0;
        listin = malloc( sizeof(char*) * 2);
    }
    listin[strlstadd_num]=newitem;
    strlstadd_num++;
    listin[strlstadd_num]=NULL;
    return listin;
}
...
char **strlstfree(char **listin)
{
    ...
    for (i=0;listin[i];i++)
    {
        if (listin[i]) free (listin[i]);
    }
    return free0(listin);
}

/**
 @brief String Argumente durchzaehlen
 @return Anzahl 0-count bzw. -1 falls keine Liste existiert (NULL)
 @param listin Pointer auf die "Stringliste"
...
/// @brief simple strcmp for qsort
// better readable, but not my choice of speed
//    const char **pa,**pb;
//    pa = (const char **)a;
//    pb = (const char **)b;
//    return( strcmp(*pa,*pb) );
int strlstcompare(const void *a, const void *b)
{
    return( strcmp( *((const char **)a), *((const char **)b)) );
}

/**
 @brief  sortieren der strlst mit qsort
 @return EXIT_FAILURE / EXIT_SUCCESS
 @param  listin zu sortieren list
...
int strlstsort(char **listin)
{
    int sz;
    if (!listin) return EXIT_FAILURE;
    sz=strlstcount(listin);
    if (sz>1) qsort(listin,sz,sizeof(char*),strlstcompare);
    return EXIT_SUCCESS;
}

/**
 @brief String aus der Liste löschen
 @return listin, wird aber nicht verändert
 @param listin Pointer auf die "Stringliste"
 @param num Nummer des zu löschenden Strings

 Bei falscher 'num', passiert nix. Ob es bei num die richtige "Nummer" ist wird
 nicht geprüft.
...
char **strlstdelete(char **listin, int num)
{
    int i;
    if (!listin) return NULL;
    if (num<0)   return listin;
    for (i=0;i<num;i++)
    {
        if (!listin[i]) return listin;                                          // return, wenn vor der Nummer ein NULL-Wert ist
    }
    free(listin[num]);                                                          // diesen freigeben
    if (num==0) if (listin[1]==NULL) return free0(listin);                      // if (num==0) und listin[1]==NULL, dann liste leer !!
    for (i=num;;i++)                                                            // übrige Liste verschieben
    {
        listin[i]=listin[i+1];                                                  // Auch NULL wird kopiert
        if (!listin[i]) break;
    }
    return listin;
}
...
/**
 @brief einzelnes zeichen nach int von einem hexwert 0123456789abcdef
 @return hex from char
 @param h der char der konvertiert wird
 ...
 ** 08.05.25 HS Ruft jetzt getFlagFromString auf und schlatet "AltVerhalten" ein
___________________________________________________________________________*/

int GetIntTrueFalsefromString(char *STRING)
{
    return getFlagFromString(STRING,0,False);
}

/**
 @brief Schaut nach ob true,false,1,0 usw. im String stehen
 @return true / false im Fehlerfall def
 @param cmd      Die zu bearbeitende Zeichenkette
 @param behavior Besondere Verfahrensweise. 0 = Altverhalten mit Fehlermeldung
 @param def      Default, falls alle Prüfungen misslingen

___[ Revision ]______________________________________________________________

 ** 08.05.25 HS Vorher GetIntTrueFalsefromString
___________________________________________________________________________*/
...
int getFlagFromString(char *cmd, int behavior, int def)
{
    if (cmd)
    {
        if (!strcmp(cmd, "Y"))        return True;
        if (!strcasecmp(cmd, "true")) return True;
        if (!strcasecmp(cmd, "on"))   return True;
        if (!strcmp(cmd, "1"))        return True;

        if (!strcmp(cmd, "N"))         return False;
        if (!strcasecmp(cmd, "false")) return False;
        if (!strcasecmp(cmd, "off"))   return False;
        if (!strcmp(cmd, "0"))         return False;
    }
    if (behavior==2) return def; // wie AltVerhalten nur ohne Fehlermeldung
    if (behavior==0)
    {
        lprintf ("Error reading TrueFalse from \"%s\"", strNotNULL(cmd));
        return def;
    }
    if (cmd)
    {
        if (!strcmp(cmd, "-"))         return False;
...
/**
 @brief ASC (auch 0x) als unsigned long zurückgeben
 @return unsigned long
 @param Kette Die zu bearbeitende Zeichenkette

  100 =
 0x64 =
 0b01100100

 0b01100100 (binär) = 100
 0b01100100 (hex)   = 47262466304

 desw. keine Binär option

___[ Revision ]______________________________________________________________

29.04.25 HS aotul wurde benötigt
___________________________________________________________________________*/

unsigned long atoul(char *Kette)
{
    int i;
    unsigned long value;
    value=0LU;
    if (Kette[0]=='0' && Kette[1]=='x')                                             // hexdigit check
    {
        for (i=2;(Kette[i] && isxdigit(Kette[i]));i++)
        {
            value = (value*16)+charhexval(Kette[i]);
        }
    }else{
        for (i=0;(Kette[i] && isxdigit(Kette[i]));i++)
        {
            value = (value*10)+(Kette[i]-'0');
        }
    }
    return value;
}
...
/**
 ...
 @return char*
 @param ins eine mögliche Beschreibung, wenn NULL dann den Inhalt von "m_PRGNAME"

___[ Revision ]______________________________________________________________

 ** 25.06.20 HS ReCreate
 ** 16.04.25 HS LLX_STR strict anstatt %I64x
___________________________________________________________________________*/
...
char *tempfilename(char *ins)
{
    ...
    for (;;)
    {
        ...
        s = strprintf ("%s" DIR_SEP  "%s." LLX_STR ".tmp", tempdir(), ins, rnd);
        if (!FileOK(s)) break;
        free(s);
    }
    return s;
...
*/ /**
 ...
 @param locgmt 1=localtime 0=gmt verwenden

___[ Revision ]______________________________________________________________

 ** 02.04.06 HS ReCreate
 ** 02.04.24 HS malloc_temp statt static
 ** 28.05.25 HS Umgestrickt um gmtime möglich zu machen
___________________________________________________________________________*/

char *ctimeinfostr(time_t TIME, int locgmt)
{
    char *rslt;
    struct tm *ti;
    switch (locgmt)
    {
        case 1: ti=localtime(&TIME);
                break;
       default: ti=gmtime(&TIME);
                break;
    }
    rslt = strprintf ("%4.4i-%2.2i-%2.2i %2.2i:%2.2i:%2.2i",
...
char *gmtinfostr(time_t TIME)
{
    return ctimeinfostr(TIME,0);
}
...
static int mon_to_day[]={ 0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334, 365 };
///< Jede Zahl stellt die Anzahl der Tage in jedem Monat dar, die zum Gesamtergebnis hinzugefügt werden.

/**
 @brief  Tag im Jahr berechnen aus Datum YYYY-MM-DD
 @return Anzahl Tage 0-366
 @param  year   volle Jahreszahl
 @param  month  0-11 Monate mit 0 beginnend für Januar
 @param  day    1-31 Tage mit 1 beginnend für den ersten Tag

 Zentral ist dabei ref mon_to_day Tabelle.

 Es wird davon ausgegangen dass alle Zahlen gültig sind.

 Zum Beispiel gibt es 31 Tage im Januar, also die erste Zahl ist 31. Es
 gibt 28 Tage im Februar (in Nicht-Schnelljahren). 31+28=59, so dass
 das Ergbnis sein wird.
...
int ydays (int year, int month, int day)
{
    int rslt;
...
//    if (day>31) day=31;
//    if (day<0) day=0;
    rslt = mon_to_day[month];
    rslt = rslt + day;
    if ( (isLeapYear(year)) && (month>=2) ) rslt++; // nach Februar im Schaltjahr
    return rslt;
}
// return mon_to_day[month]+day+((isLeapYear(year))&&(mon>=2))?1:0;
...
/**
 @brief FQN des gegebenen Filenamens auflösen
 @return String des fqn oder NULL if Fail
 @param Kette der Pfad (File) das aufgelöst werden soll

 Wenn das erste Zeichen ein '~' ist wird das es um das HomeDir erstezt.

 Wichtig: Es findet keine Prüfung der angegebenen Datei statt. Es wird
 nur logisch aufgelöst. Somit können auch Links nicht berücksichtigt sein.

 @code
 // Usage: PATH="/hs/src/tester/tooltest";
 char *myfqn=Cresolv_path("/usr/var/../bin/ls");
 // myfqn="/usr/bin/ls"
 // /usr/bin/ls               => /usr/bin/ls
 // ../app.exe                => /hs/src/tester/app.exe
 // /usr/share/../bin/ls      => /usr/bin/ls
 // /usr/share/../../bin/ls   => /bin/ls
 // ../../../app.exe          => /hs/app.exe
 // ./app.exe                 => /hs/src/tester/tooltest/app.exe
 // ./../app.exe              => /hs/src/tester/app.exe
 // /../app.exe               => /app.exe
 // /usr/./app.exe            => /usr/app.exe
 // /./app.exe                => /app.exe
 // ~/.config/.superapp.conf  => /home/hesti/.config/.superapp.conf
 @endcode

___[ Revision ]______________________________________________________________

 ** 08.04.25 HS Create
 ** 20.05.25 HS ".." und "." als einziger Parameter und ".." "." innerhalb
                und am Ende richtig auslösen
 ** 01.06.25 HS root wie / kann nicht aufgelöst werden
___________________________________________________________________________*/

/* Windows Umsetzung:
"C:" .. fehlt ansonsten ist vieles sehr gleich
ODER (from: https://learn.microsoft.com/de-de/windows/win32/api/fileapi/nf-fileapi-getfullpathnamea )
retval = GetFullPathName(argv[1],
         BUFSIZE,
         buffer,
         lppPart);
Parameter:
[in] lpFileName     Der Name der Datei.
...
[in] nBufferLength  Die Größe des Puffers, um die mit Null beendete Zeichenfolge für das Laufwerk und den Pfad zu empfangen, in TCHARs.
[out] lpBuffer      Ein Zeiger auf einen Puffer, der die mit Null beendete Zeichenfolge für das Laufwerk und den Pfad empfängt.
[out] lpFilePart    Ein Zeiger auf einen Puffer, der die Adresse (innerhalb lpBuffer) der endgültigen Dateinamenkomponente im Pfad empfängt.
...
char *CresolvPath(char *Kette)
{
#ifdef OS_LINUX
    int  posi;                                                                      // pos in obuf
    char *obuf;                                                                     // Ergebnis
    int  loop;                                                                      // zum Kopieren eines teilpath
    char *path;                                                                     // Kopie von Kette ([const] Kette bleibt unangetastet)
    char *slash;                                                                    // zeigt auf einen Slash

    path = malloc_temp_strcpy(Kette);                                               // Kopie anlegen
    obuf = malloc_temp(PATH_MAX+1);                                                 // Ergebnis vorbereiten
    posi = 0;                                                                       // Position im Ergebnis

    if (path[0]==0)                                                                 // Etwas muss drin stehen
    {
        return NULL;
    }else if (path[0] == '/')                                                       // Wenn mit Slash beginnt, dann
    {
        if (path[1]==0) return path;                                                // Wenn nur / angegeben, dann war es das schon
        path++;                                                                     // Input auf den nächsten, Überlese den ROOT-Slash
    }else if (path[0] == '~')                                                       // Wenn $HOME
    {
        strcpy (obuf, strNotNULL(RCunivault("%HOME%")));                            // HOME ist home ist $HOME
        posi=strlen(obuf);                                                          // Länge; GetCWD liefert das Ergebnis in obuf
        path++;                                                                     // überlese '~'
        while (path[0]=='/') path++;                                                // danach ohne '/', wird im nächsten Schritt angehängt
    }else{
        //if (!getcwd(obuf, PATH_MAX)) return NULL;
        if (GetCWD(obuf)) return NULL;                                              // Wenn nicht ge"Rootet", dann geht es ab dem aktuellen Pfad weiter
        posi=strlen(obuf);                                                          // Länge; GetCWD liefert das Ergebnis in obuf
    }
    if (path[0] == 0)
    {
        obuf[posi]=0;
        return obuf;                                                                // Wenn Input zu Ende, dann NULL, das ist ein Fail
    }
    obuf[posi]='/';                                                                 // Ein Slash muss ans Ende
    posi++;

    for(;;)
    {
        if (path[0]=='.')                                                           // dir aktuelle/upper brauchen einen slash am Ende
        {
            if (!strcmp(path,"."))  strcat(path,"/");
            if (!strcmp(path,"..")) strcat(path,"/");
        }
        if (path[0]==0) slash=NULL;                                                 // Supersave aber slash = strchr(path[0]?path:"",'/'); wäre auch gegangen
        else            slash=strchr(path,'/');                                     // Der leichteren Lesbarkeit halber
        if (slash)
        {
            slash[0]=0;                                                             // Wenn gefunden, dann dort das '/' entfernen path ist jetzt nur noch ein Teil
            if (!strcmp(path,".."))                                                 // Wenn "dir up", dann
            {
                if (posi>0) posi--;                                                 // vor die letzte 0, bzw. die vorbereitung zum nächsten Zeichen
                if (posi>0) posi--;                                                 // letztes Zeichen vermutlich ein '/' muss auch wech
                while(obuf[posi]!='/')                                              // und nach dem vorherigen '/' suchen
                {
                    posi--;
                    if (posi<0)                                                     // zu klein, dann root
                    {
                        posi=0;                                                     // posi
                        obuf[posi]='/';                                             // root
                        break;                                                      // ende der while
                    }
                }
                posi++;                                                             // hinter des Zeichen
                obuf[posi]=0;                                                       // markieren??
                path=slash;                                                         // Input
                path++;                                                             // hinter der slash
                continue;                                                           // weiter machen
            }else if (!strcmp(path,"."))                                                  // nur ein Punkt wird ignoriert
            {
                path=slash;                                                         // Skip to Slash
                path++;
                continue;
            }
        }
        for (loop=0;;loop++,posi++)                                                 // Alles andere kopieren
        {
            obuf[posi]=path[loop];                                                  // Bereich bis Slash oder den ganzen Rest, wenn kein Slash
            if (obuf[posi]==0) break;
        }
        if (!slash) break;                                                          // gab es im Input einen Slash, wenn nicht, dann fertig

        obuf[posi]='/';                                                             // Ansonsten wird dieser im Ausgabebereich wieder hergestellt
        posi++;
        path = slash;
        path++;
    }
...
#endif
#ifdef OS_WINDOWS
    lprintf ("Windows part of Cresolv_path has NULL result")
    return NULL;
#endif
}
...
/**
 ...
 path wird bestenfalls kleiner, es werden keine zeichen hinzugefügt
...
int RCreadPath(int opt, char *rc_Fullname, char *subpath, char *filename)
{
    ...
    //: abfangen? vielleicht in HS_DEBUG?, waere dann ein "interner Assert"
...
/**
 ...
 @param RCNodes Nodes, die durchsucht werden sollen
 @param var Variable die gefunden werden soll
 @param sec eine mögliche section, wenn NULL, dann wird Sie weggelassen

 @code
    return RCfind_in_Section(RCNodes, var, "usersection");
 @endcode
...
char *RCfind_in_Section(void *RCNodes, char *var, char *sec, int num)
{
    char *r;
    char *nvar;
    if (!sec) return RCfindNum(RCNodes, var, num);
    nvar = malloc0(strlen(var)+strlen(sec)+16);
    sprintf(nvar,"%s.%s", sec, var);
    r=RCfindNum(RCNodes, nvar, num);
    free(nvar);
    return r;
}
...
void *Node_Add (void* StartNode, void *Data, int direction)
{
    ...
    if (direction==false) // Node_Add_Insert (false und true loesung?)
    {
